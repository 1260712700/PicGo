...
import com.jp.mapper.RolePermissionMapper;
...
public class JwtUtil {
    ...
    private boolean deleteToken(String uuid) {
        ...
        if (this.isInvalidToken(uuid))
            return false;
        // 删除
        redisCache.deleteObject(RedisConstant.JWT_WHITE_LIST + uuid);
        return true;
    }
    ...
    private boolean isInvalidToken(String uuid) {
        // 判断是否在redis中(白名单)
        return !Boolean.TRUE.equals(redisCache.isHasKey(RedisConstant.JWT_WHITE_LIST + uuid));
    }
...
//                .withClaim("authorities", details.getAuthorities().stream().map(GrantedAuthority::getAuthority).toList())
                ...
                .sign(algorithm);
        // 存入redis
        redisCache.setCacheObject(RedisConstant.JWT_WHITE_LIST + uuid, jwt, (int) (expire.getTime() - now.getTime()), TimeUnit.MILLISECONDS);
        return jwt;
    }
